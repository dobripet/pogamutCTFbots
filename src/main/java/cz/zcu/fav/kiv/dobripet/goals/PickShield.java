...
public class PickShield extends Goal{
    ...
    @Override
    public void perform() {
        bot.getBot().getBotName().setInfo("PICK SHIELD" + item.getType().getName());
        bot.pickItem(item);
    }
    ...
    public double getPriority() {
        ...
        for (Item i : items) {
            ...
            if(i.getType().equals(UT2004ItemType.SHIELD_PACK)){
                ...
                c = (d - ((double)bot.getInfo().getLowArmor())/50 * d) * Priority.SHIELD_PRIORITY;
            }
        ...
        // hard limit
        if (maxPriority > Priority.SHIELD_MAX_PRIORITY) {
            return Priority.SHIELD_MAX_PRIORITY;
        }
        return maxPriority;
    }
